### This file includes an updated version of the "PAMGuide_Meta" function that
### was originally published as part of the PAMGuide package by Nathan Merchant.
###
### We have add new functionality to the code, applied more consistent code
### formatting throughout the script, introduced various checks to ensure that
### the function can be run using parameter values provided by the user, added
### various `message()` and `cat()` outputs to provide a record of those to the
### user and to assist in debugging, and allow the function to return various
### summary statistics for particular windows of time of interest.
###
### For the most part, our code additions are documented with comments indicated
### by three hashtags (`###`), although we have also directly modified in minor
### ways some of the original code to utilize more consistent formatting (e.g.,
### by adding white space around operators, inserting spaces for tabs, checking
### indentation levels for code blocks, and making capitalization and spacing with
### output to console more consistent).
...
# Merchant ND, et al (2015). Measuring acoustic habitats. Methods in Ecology
# and Evolution 6: 257–265. https://doi.org/10.1111/2041-210X.12330
...
### Revisions: Anthony Di Fiore. Last modified 19 Aug 2023

# PREREQUISITES: "PAMGuide_Meta_revised.R" uses the package {tuneR} and
# {tidyverse}, which can be installed using the R Package Installer.

## Install {tuneR} and {tidyverse} if not installed---------------------------------------------------
# if {tuneR} is not installed
if (!is.element('tuneR', installed.packages()[, 1])) {
...
if (!is.element('tidyverse', installed.packages()[, 1]))  {
  ...
  options(repos = r)
  rm(r)
  install.packages('tidyverse', dep = TRUE)    # install {tidyverse}
  require('tidyverse', character.only = TRUE)}

library(tuneR)                                 # load {tuneR}
library(tidyverse)                             # load {tidyverse}

## Begin PAMGuide_Meta---------------------------------------------------------

PAMGuide_Meta_revised <- function(fullfile,..., atype = "PSD", plottype = "Both", envi = "Air", calib = 0, ctype = "TS", Si = -159, Mh = -36, G = 0, vADC = 1.414, r = 50, N = Fs, winname = "Hann", lcut = Fs/N, hcut = Fs/2, timestring = "", outdir = dirname(fullfile), outwrite = 0, disppar = 1, welch = "", chunksize = "", linlog = "Log", StartTime = NA, CallOnset = NA, seconds = NA, windowDirection = NA, channel = 1) {

  ### We have added the following new arguments and default values to the function:
  ###  - StartTime (with default NA) as POSIXct time
  ###  - CallOnset (with default NA) as POSIXct time
  ###  - seconds (with default NA) as integer
  ###  - windowDirection (with default NA) as character
  ###  - channel (with default 1) as numeric
  ###
  ### If values for these new arguments are not specified, then the function
  ### should run equivalently to the original PAMGuide_Meta function
  ###
  ### Below, we describe each new argument:
  ###  - StartTime = real start time of audio file
  ###  - CallOnset = time *within file* that a vocalization (or other time
  ###    segment) of interest begins
  ###  - seconds = length, in seconds, of the time window to analyze from the
  ###    audio file; this parameter value is rounded to the nearest integer to
  ###    pass to the `readWave()` function; rounded seconds should be ≥ 2
  ###  - windowDirection = whether the window to be analyzed should be extracted
  ###    from "before" the CallOnset time (e.g., for characterizing ambient
  ###    noise before a call) or from "after" the CallOnset time (e.g., for
  ###    characterizing the initial part of a call of interest)
  ###  - channel = 1 (for left) or 2 (for right) channel for recorders with
  ###    multiple microphones that can record in stereo

  ### We define a new variable, onsetSec, which is calculated as the *time in
  ### seconds* from the start of a file when the vocalization (or other time
  ### segment of interest) begins. Note that we force `difftime()` units to be
  ### in "seconds" and then convert this to be a numeric variable rather than
  ### to have class `difftime`
  ...
  ### New line to indicate start of batch file processing
  message("### Begin processing audio file\n")

  ### New code block to check that rounded window length, if specified, is at
  ### least 2 seconds
  if (!is.na(seconds)) {
    seconds <- round(seconds, 0)
    if (seconds < 2) {
      message(">>> Cannot run function... Window length should be at least 2 sec or not specified\n")
      return(NA)
    }
  }
  ...
  ### New code block to see if file exists and throw an error message if it does not
  message(paste0("Audio file name: ", ifile))
  if (!file.exists(fullfile)) {
    message()
    message(paste0('>>> Cannot run function... "', fullfile, '" not found\n'))
    return(NA)
  }

  ### New code block to get and report full length of file in seconds
  fIN <- readWave(fullfile, header = TRUE)
  fileLength <- fIN[[4]]/fIN[[1]]
  message(paste0('Full length of audio file: ', sprintf('%.03f', fileLength), " sec"))
  ### Determine EndTime for file is StartTime is specified
  if (!is.na(StartTime)) {
    EndTime <- StartTime + fileLength
    message(paste0('Audio file covers: ', format(StartTime, "%H:%M:%S"), " to ", format(EndTime, "%H:%M:%S")))
  }

  message()

  ### The code block below was changed to potentially read a window of a specified
  ### number of "seconds" from the file, starting that amount of time either
  ### "before" or "after" a specified CallOnset time
  ###
  ### It also adds options to `cat()` out various information about the file
  ###
  ### If CallOnset, StartTime, seconds, and windowDirection parameters are all
  ### not specified, run the function on the entire file; this should yield same
  ### results as original `PAMGuide_Meta()` function

  if (is.na(CallOnset) & is.na(StartTime) & is.na(seconds) & is.na(windowDirection)) {
    message(paste0('>>> Running function on entire file\n'))
    fIN <- readWave(fullfile,header = TRUE)  # read file header
  ### If all four of these parameters are specified, then run the function on
  ### the specified window of time "before" or "after" CallOnset
  } else if (isTRUE(!is.na(CallOnset) & !is.na(StartTime) & !is.na(seconds) & windowDirection == "before") == TRUE) {
    ### Check if CallOnset indeed has a window of "seconds" before it in file
    if (difftime(CallOnset, StartTime, units = "secs") < seconds) {
      message(paste0('>>> Cannot run function... CallOnset is less than ', seconds, ' sec after start of audio file'))
      ### The following lines can be uncommented to print out additional info
      ...
      # cat('Window length is:', seconds, 'sec\n')
      cat("\n")
      return(NA)
    }
    message(paste0('>>> Running function on a ', seconds, ' sec window extracted from file: ', format(CallOnset - seconds, "%H:%M:%S"), ' to ', format(CallOnset, "%H:%M:%S"), '\n'))
    cat('StartTime of sample is:', ifelse(!is.na(StartTime), format(StartTime, "%H:%M:%S"), NA), '\n')
    cat('CallOnset is:', ifelse(!is.na(CallOnset), format(CallOnset, "%H:%M:%S"), NA), '\n')
    cat('Window is:', seconds, 'sec before Call Onset\n')
    fIN <- readWave(fullfile, header = TRUE, from = onsetSec - seconds, to = onsetSec, units = "seconds")
  } else if (isTRUE(!is.na(CallOnset) & !is.na(StartTime) & !is.na(seconds) & windowDirection == "after") == TRUE) {
    ### Check if CallOnset indeed has a window of "seconds" after it in file
    if (difftime(EndTime, CallOnset, units = "secs") < seconds) {
      message(paste0('>>> Cannot run function... CallOnset is less than ', seconds, ' sec before end of audio file'))
      ### The following lines can be uncommented to print out additional info
      # cat('StartTime of sample is:', ifelse(!is.na(StartTime), format(StartTime, "%H:%M:%S"),NA), '\n')
      # cat('CallOnset is:', ifelse(!is.na(CallOnset), format(CallOnset, "%H:%M:%S"),NA), '\n')
      # cat('Window length is:', seconds, 'sec\n')
    ...
    message(paste0('>>> Running function on ', seconds, ' sec window extracted from file: ', format(CallOnset, "%H:%M:%S"), ' to ', format(CallOnset + seconds, "%H:%M:%S"), '\n'))
    cat('StartTime of sample is:', ifelse(!is.na(StartTime), format(StartTime, "%H:%M:%S"), NA), '\n')
    cat('CallOnset is:', ifelse(!is.na(CallOnset), format(CallOnset, "%H:%M:%S"), NA), '\n')
    cat('Window is:', seconds, 'sec after Call Onset\n')
    fIN <- readWave(fullfile, header = TRUE, from = onsetSec, to = onsetSec + seconds, units = 'seconds')
  ### And if these four parameters are incompletely specified, then report that
  ### the function cannot run
  } else {
    message(paste0('>>> Cannot run function... Parameters are incompletely specified'))
    ### The following lines can be uncommented to print out additional info
    # cat('StartTime of sample is:', ifelse(!is.na(StartTime), format(StartTime, "%H:%M:%S"),NA), '\n')
    # cat('CallOnset is:', ifelse(!is.na(CallOnset), format(CallOnset, "%H:%M:%S"),NA), '\n')
    # cat('Window length is:', seconds, 'sec\n')
    # cat('Window direction is:', windowDirection, '\n')
    cat("\n")
    return(NA)
  }

  Fs <- fIN[[1]]                        # sampling frequency
  cat('Sampling frequency:', Fs, '\n')  ### New line to print sampling frequency
  Nbit <- fIN[[3]]                      # bit depth
  cat('Bit depth:', Nbit, '\n')         ### New line to print bit depth

  if (is.na(CallOnset) & is.na(StartTime) & is.na(seconds)) {
    xl <- fIN[[4]]                       # length of full file in samples
    cat('Length of full file in samples:', xl, '\n\n')  ### New line to print number of samples in full file
  } else {
    xl <- Fs * seconds                   ### Calculate xl based on the length
                                         ### of the "seconds" window rather than
                                         ### simply taking that from the file
                                         ### header
    cat('Length of', seconds, 'sec from file in samples:', xl, '\n\n')  ### New line to print number of samples from file corresponding to the length of the time window of interest
  }
  ...
  if (disppar == 1) {
    ...
    cat('Time segment length:', N, 'samples =', N/Fs, 'sec\n')
  ...
  for (q in 1:nchunks) {
    ### We modified the `if (nchunks==1)` routine...
    if (nchunks == 1) {
      # t1=proc.time()  # start timer
      # cat('Loading input file... ')
      ### The `xbit` code below was modified to read just a window of "seconds"
      ### from the file, either "before" or "after" the specified CallOnset time
      if (is.na(CallOnset) & is.na(StartTime) & is.na(seconds) & is.na(windowDirection)) {
        ### If all of these four parameters are not specified, read the full file
      ...
      ### We added the `if` statements below to pick which channel to use...
      ### Uncommenting the `print()` line below confirms that a data structure
      ### with class `Wave` has both an @left and @right channel
      ### print(str(xbit))
      if (channel == 1) {xbit <- xbit@left/(2^(Nbit - 1))}
      if (channel == 2) {xbit <- xbit@right/(2^(Nbit - 1))}
      # convert to full scale (+/- 1) via bit depth

    ### We did not modify the following code block for cases where nchunks > 1,
    ### so if an argument of nchunks > 1 is passed, the code below is *not*
    ### accurate.
    ### To correct this, the `xbit <- readWave()` line would need to be altered.
    ### We did not consider how best to do that because it would involve breaking
    ### up the "seconds" window into chunks.
    ...
    if (plottype == 'Stats' | plottype == 'Both') {
      if (atype != 'Broadband') {
        ...
        if (atype == 'TOL') {
          ...
          mtext(text = YLAB, side = 2, line = 2)
          cat('done in', (proc.time() - tstat)[3], 'sec\n')
        }
      }  ### End of `atype != 'Broadband' block
         ### This modified code moved the closing brace here from line 775 to
         ### separate `atype != 'Broadband'` from `atype == 'Broadband'` output

      if (atype == 'Broadband') {  ### Indentation reduced here from original file
        ...
        ### This is a new code block, copied from the VIEWER.R function to
        ### calculate mean RMS levels as part of `atype == 'Broadband' output
          ...
          if (calib == 1) {
            ...
            cat('SEL =', sprintf('%.1f', SEL), 'dB re', pref, 'uPa^2 sec\nNote: for SEL measurements, set r = 0 (window overlap) and use default N\n\n')
          } else {
            ...
            cat('Relative normalised SEL =', sprintf('%.1f', SEL), 'dB\nNote: for SEL measurements, set r = 0 (window overlap) and use default N\n\n')
          }
          ...
          ### We added the lines below to return a more complete `BBresults`
          ### object with various elements if `atype = 'Broadband'`
          BBresults <- list(RMSlev = RMSlev,
                            medlev = medlev,
                            modelev = modelev)
          ### The line above generates of list of data elements as part of the
          ### `BBresults` object
      }  ### End of `atype == 'Broadband'` block

      if (M < 1000 & atype != 'Broadband') {
        ### This code block perhaps should really be nested within the
        ### `atype != 'Broadband'` block, from lines 637 to 699
          cat('Too few time segments (M =', M, ', i.e. <1000) for SPD analysis: for SPD, use a longer file or shorter time segment length (N)\n\n')
        }
        ### There seems to be misleading terminology in this warning...
        ### M isn't the number of time segments but rather the number of data
        ### points (or rows) in final matrix, N is the number of time segments,
        ### and N/Fs = length of each time segment in seconds
        ###
        ### M *should* be equivalent to the length of time being considered
        ### (i.e., a range of seconds) divided by the length of each time
        ### segment (N/Fs) * 2, or `seconds * 2 * Fs / N`
      }
  # } This was the original closing brace that we shifted to line 709
  }

  # return(A)  # return output array
  ### This is the raw data matrix generated in line 627, before any statistical
  ### analysis. This is also what is returned from the original PAMGuide_Meta
  ### function.
  ###
  ### Our revised function instead allows the function to return calculated
  ### average RMS results (and other summary statistics) for when `atype ==
  ### 'Broadband'`
  ###
  ### The line below generates the list of data elements to return...
  results <- list(A = ifelse(!is.na(A), A, NA),
                  RMSlev = ifelse(exists("BBresults"), BBresults[["RMSlev"]], NA),
                  medlev = ifelse(exists("BBresults"), BBresults[["medlev"]], NA),
                  modelev = ifelse(exists("BBresults"), BBresults[["modelev"]], NA))
  return(results)  # return a list of data elements
}
