...
# Step 5 data
d100 <- read_csv("Data/Playback_100dB_Script1.csv", col_names = TRUE) # Load in file
...
PAMGuide then uses this to determine how many seconds into the audio file it needs to start the 5-second measurement. Here we use short clips of the two duets recorded clostst to the Song Meters to demonstrate the modified PAMGuide code.
```{r}
...
homerange1 <- homerange1 %>% rowwise() %>% 
  mutate(CalibratedSPL = PAMGuide_Meta_revised(fullfile = paste0("Audio/SourceLevel/", ... = FileName), 
                                           atype= "Broadband",
                                           StartTime=StartTime,
                                           CallOnset=CallOnset,
...
## Step 5: Attenuation (by Tony Di Fiore)
...
First we create two separate data frames, one for expected SPL data and one for observed SPL data.
```{r}
exp_d100 <- d100 %>% filter(Group == "exp")
obs_d100 <- d100 %>% filter(Group == "obs")
```

```{r}
exp100 <- lm(data = exp_d100, sourcelevel ~ log(pythagoras_distance)) # Fit linear model to expected data.
...
```{r}
# Add in 38.2 and 48.4 to get interpolated values for absolute received level at these distances
dist100a <- c(1, 10, 25, 38.2, 48.4, 50, 75, 100, 125, 150, 175, 200)

excess_attenuation100a <- (broom::tidy(exp100)$estimate[1] + broom::tidy(exp100)$estimate[2] * log(dist100a)) - (broom::tidy(obs100)$estimate[1] + broom::tidy(obs100)$estimate[2] * log(dist100a))

excess_attenuation100a <- tibble(distance = dist100a, excess_attenuation = excess_attenuation100a)
...
panelC <- ggplot(data = exp_d100) +
  geom_point(aes(x = pythagoras_distance, y = sourcelevel, color = "Theoretical")) +
  geom_smooth(aes(x = pythagoras_distance, y = sourcelevel, color = "Theoretical"), method = "lm", formula = y ~ log(x), se = FALSE, linetype = 1) +
  ...
  scale_color_manual(name = "Series", breaks = c("Theoretical", "Observed"), values = c("Theoretical" = "red", "Observed" = "blue")) +
